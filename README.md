...
See [docs/USE-CASES.md](docs/USE-CASES.md) for the fuller picture — the hero flow
above plus a real-world, non-technical operating pattern, written up in the open.
...
**Known issues:** we log gaps with the same discipline as the gates. Connector
status, the install step, the eval refusal rate, the reads-unauthenticated-on-localhost
design choice, and a full graph-data teardown are in
[docs/KNOWN-ISSUES.md](docs/KNOWN-ISSUES.md).

## What's built vs. what's next

**Built:** hybrid retrieval (sqlite-vec + FTS5, RRF) with per-chunk citations · cross-model
validator gate · receipt + cost ledger · a read-only graph panel over a known-thin index
(coverage and precision numbers, root cause, and the gated rebuild are in
[Known Issues](docs/KNOWN-ISSUES.md)) · episodic session recall ·
